...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("montecarlo"))
        return false;
...
{
    // Convert montecarlo:// to montecarlo:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("montecarlo://"))
    {
        uri.replace(0, 12, "montecarlo:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MonteCarlo.lnk";
}
...
{
    return GetAutostartDir() / "montecarlo.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MonteCarlo-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  montecarlo-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("MonteCarlo-Qt"));
